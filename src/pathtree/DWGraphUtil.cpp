    ...
    map<int, vector<int>>& sccmap, int& scc)
{
    ...
    for (eit = el.begin(); eit != el.end(); eit++) {
        if (!g.vl[g.target(*eit)].visited) {
            ...
            order[vid].second
                = min(order[g.target(*eit)].second, order[vid].second);
        } else if (find(sn.begin(), sn.end(), g.target(*eit)) != sn.end()) {
            order[vid].second
                = min(order[g.target(*eit)].first, order[vid].second);
        }
    }
...
//	cin >> ch1;
        ...
        for (vit = sccvec.begin(); vit != sccvec.end(); vit++) {
            ...
            el = ng.out_edges(*vit);
            for (eit = el.begin(); eit != el.end(); eit++) {
                //	if (find(sccvec.begin(), sccvec.end(),
                // g.target(*eit))
                //!= sccvec.end()) {
                ...
                //	ng.addEdge(maxid, g.target(*eit),
                // g.edgeOpMap[*eit].weight, *eit); ng.addEdge(maxid,
                // g.target(*eit), g.weight(*eit), *eit);
                ...
                newnode.edgeid = *eit;
                edgemap.insert(make_pair(make_pair(maxid, newnode),
                    make_pair(*vit, newnode)));
            }
...
//	cin >> ch;
            ...
            for (eit = el.begin(); eit != el.end(); eit++) {
                ...
                //	ng.addEdge(ng.edgeOpMap[*eit].trg, maxid, y1,
                //*eit);
                ...
                // Dec 15
                // change ng.source(*eit) to g.source(*eit) such that
                // mapping the original edges to new graph ng
                ...
                // Dec 15
                // change ng.target(*eit) to ng.source(*eit); change the
                // order of update edgemap and add edge operation
...
// for test
    ...
    for (rvit2 = list_s.rbegin(); rvit2 != list_s.rend(); rvit2++) {
        ...
        if (maxBranch.in_edges(*rvit2).size() == 0) {
            ...
            // for test
#ifdef DEBUG
            cout << "search edge " << maxBranch.source(el.front()) << "->"
                 << *rvit2 << endl;
#endif
            ...
            DWVertexProp dwep2
                = maxBranch.edge(maxBranch.source(el.front()), *rvit2);
            ...
            if (newedge == edgemap.end()) {
                ...
                for (iter = edgemap.begin(); iter != edgemap.end(); iter++)
                    cout << iter->first.first << "->" << iter->first.second.id
                         ...
                         << " eid " << iter->second.second.edgeid << endl;
                cout << "in search edge " << edge.first << "->" << edge.second.id
                     << " eid " << edge.second.edgeid << endl;
                exit(0);
            }
#endif
        ...
        if (maxBranch.out_edges(*rvit2).size() > 0) {
            ...
            for (eit = el.begin(); eit != el.end(); eit++) {
                ...
                if (newedge == edgemap.end()) {
                    ...
                    cout << "search edge " << edge.first << "->"
                         << edge.second.id << " eid " << edge.second.edgeid
                         << endl;
                    exit(0);
                }
#endif
...
bool DWGraphUtil::checkBranching(DWGraph& graph, DWGraph& branch)
{
    ...
    if (!result) {
        cout << "error: More than one incoming edges!" << endl;
        return false;
    }
    int index = 0;
    vector<int> sn;
        ...
        for (j = 0; j < n; j++) {
            if (i != j) {
                ...
                if (rand_num < threshold) {
                    if (checkEdge.find(pair<int, int>(i, j))
                        == checkEdge.end()) {
                        weight = rand() % n;

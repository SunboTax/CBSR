    ...
    vector<int>& ts)
{
    ...
    for (rvit = ts.rbegin(); rvit != ts.rend(); rvit++) {
        ...
        /*
        if (pred[*rvit].size() > g.num_vertices()) { cout << "error" <<  endl;
...
void GraphUtil::mergeSCC(Graph& g, int* on, vector<int>& reverse_topo_sort)
{
    ...
    while (mit != sccmap.end()) {
        ...
        for (; mit != sccmap.upper_bound(num_comp); ++mit) {
            ...
            for (vit = vec.begin(); vit != vec.end(); vit++) {
                for (vit1 = outlist[*vit].begin();
                     vit1 != outlist[*vit].end();)
                    if (*vit1 == mit->second)
    ...
    vector<vector<int>>& pathMap)
{
    ...
    /*
            ...
            int i = 0;
            for (mlit = pathMap.begin(), i=0; mlit != pathMap.end(); mlit++,
       i++) { path1 = *mlit; cout << "path [" << i << "]: "; for (lit =
       path1.begin(); lit != path1.end(); lit++) cout << *lit << " "; cout <<
       endl;
            }
    */
}
    ...
    vector<int> reverse_topo_sort)
{
    ...
    for (rit = reverse_topo_sort.rbegin(), i = 0;
         rit != reverse_topo_sort.rend(); rit++) {
        k = *rit;
        if (visited[k])
            continue;
        visited[k] = true;
